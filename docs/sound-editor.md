...
With CS2 running on the configured NetConsole port, the editor can send `snd_sos_stop_all_soundevents` followed by `snd_sos_start_soundevent <name>` to preview the selected event.

## Audio files

Use the [Audio Editor](#audio-editor) to edit WAV source audio, then add the relevant sound path to the event's properties.
